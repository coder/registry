...
function append_extensions_to_settings_json() {
    ...
    # Add theme and selectedAuthType fields with better defaults
    jq '.theme = "Default" | .selectedAuthType = "gemini-api-key" | .geminicodeassist.agentYoloMode = true' "$SETTINGS_PATH" > "$TMP_SETTINGS" && mv "$TMP_SETTINGS" "$SETTINGS_PATH"
