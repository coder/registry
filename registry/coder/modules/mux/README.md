...
### Restart After Signal-Based Exits

Enable automatic restarts after Mux is killed by a signal (for example, `SIGKILL` from an external kill or the OOM killer). The launcher waits for `restart_delay_seconds`, removes `~/.mux/server.lock`, and starts Mux again. Set `max_restart_attempts` to stop retrying after a fixed number of signal-triggered restarts, or leave it at `0` for unlimited retries.
...
- Set `restart_on_kill = true` to wait `restart_delay_seconds`, remove `~/.mux/server.lock`, and restart Mux after signal-based exits
- Set `max_restart_attempts` to cap signal-triggered restarts, or leave it at `0` for unlimited retries
