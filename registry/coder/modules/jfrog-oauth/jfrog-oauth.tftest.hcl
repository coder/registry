...
run "test_with_pnpm_package_manager" {
  ...
  assert {
    condition     = strcontains(resource.coder_script.jfrog[0].script, "jf pnpmc --global --repo-resolve \"global\"")
  ...
  assert {
    condition     = strcontains(resource.coder_script.jfrog[0].script, "if [ \"true\" == \"true\" ] && ! command -v jf")
    error_message = "pnpm configuration should require the JFrog CLI even when automatic CLI setup is disabled"
  }
}
...
run "test_with_npm_and_pnpm_package_managers" {
  ...
  assert {
    condition     = length(regexall("cat << EOF > ~/.npmrc", resource.coder_script.jfrog[0].script)) == 1
